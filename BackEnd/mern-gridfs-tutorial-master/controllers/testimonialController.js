...
*/
 router.post("/:id", (req, res) => {
    ...
    Testimonial.findById(id, (err, testimonial) => {
      if (!testimonial) {
        ...
        testimonial.testimonialstitle = req.body.testimonialstitle;
        testimonial.testimonialscontent = req.body.testimonialscontent;
 ...
 router.delete('/:id', (req, res) => {
    ...
    Testimonial.findByIdAndRemove(req.params.id, (err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Testimonial Delete :' + JSON.stringify(err, undefined, 2)); }
    });
});
