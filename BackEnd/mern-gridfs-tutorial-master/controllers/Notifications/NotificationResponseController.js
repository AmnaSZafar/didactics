...
var { NotificationResponseSchema } = require('../../models/Notifications/NotificationResponseSchema')

 //Now to use router.ger to use properties of the schema like find collection, get collection, etc

 //To use this get requeest we need to type http://localhost:3001/employees/, this is coming from index.js 
 //Where its mentioned /employees is the router

 router.post('/', (req, res) => {
     NotificationResponseSchema.find((err, doc) => {
         if (!err) { res.send(doc); }
         else { console.log('Error in Retrieving NotificationResponseSchema :' + JSON.stringify(err, undifines, 2)); }
     });
 ...
 router.post('/approved', (req, res) => {
    NotificationResponseSchema.find({ "status" : "1" },(err, doc) => {
        if (!err) { res.send(doc); }
        else { console.log('Error in Retrieving NotificationResponseSchema :' + JSON.stringify(err, undifines, 2)); }
    });
 ...
 router.post("/:id", (req, res) => {
    ...
    NotificationResponseSchema.findById(id, (err, notification) => {
 ...
 router.post('/create', (req, res) => {
     var emp = new NotificationResponseSchema({
 ...
 /*router.post('/:id', (req, res) => {
    ...
    NotificationResponseSchema.findByIdAndUpdate(req.params.id, { $set: emp}, { new: true }, (err, doc) =>{
        //Checking for errors
        //If error not found, sending response to the doc
        if(!err) {res.send(doc); }
        else { console.log('Error in NotificationResponseSchema Update :' + JSON.stringify(err, undefined, 2)); }
    });
...
*/
 router.post("/:id", (req, res) => {
    ...
    NotificationResponseSchema.findById(id, (err, notification) => {
      if (!notification) {
        ...
        notification.responseTitle = req.body.notificationTitle;
        notification.responseContent = req.body.notificationContent;
 ...
 router.delete("/delete/:id", (req, res) => {
  ...
  NotificationResponseSchema.findByIdAndRemove(id,(err, doc) => {
      if (!err) { res.send(doc); }
      else { console.log('Error in Retrieving NotificationResponseSchema :' + JSON.stringify(err, undifines, 2)); }
  });
