...
export default class AudioRecorder extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <div>
          <input className="btn border form-control border-rounded" type="file" onChange={this.fileChanged.bind(this)}/>
          <br /><br />
          <button className="btn border border-rounded" onClick={this.uploadFile.bind(this)}>Upload</button>
          
          <button className="btn border border-rounded"  onClick={this.start} disabled={this.state.isRecording}>Record</button>
          <button className="btn border border-rounded" onClick={this.stop} disabled={!this.state.isRecording}>Stop</button>
          <br /><br />
          <audio src={this.state.blobURL} id="player" controls="controls" className=""  />
          <br />

          <div className="containerSass"> 
          <button className="btn rounded-circle p-3 shadow-sm" onClick={this.handlePlay}>
          ...
          <button className="btn rounded-circle p-3 shadow-sm" onClick="document.getElementById('player').pause()">&nbsp; <i className="fa fa-pause"></i> &nbsp;</button> 
          <button className="btn rounded-circle p-3 shadow-sm" onClick="document.getElementById('player').volume += 0.1">Vol +</button> 
          <button className="btn rounded-circle p-3 shadow-sm" onClick="document.getElementById('player').volume -= 0.1">Vol -</button> 
          </div>
          {/*}
          <div className="containerSass">
          <button className="btnSass play-pause buttonSass">
            <div className="iconSass-container">
              <svg className="iconSass play">
                <use xlinkHref="#play" />
              </svg>
