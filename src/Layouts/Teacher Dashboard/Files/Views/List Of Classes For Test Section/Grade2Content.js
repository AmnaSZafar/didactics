...
function Grade2ContentTestForTeacher() {
  ...
  const handleOnClickEdit = (data) => {
    if ((data.questiontype === "Text") && (data.answertype === "English Text")){
      history.push({
        pathname:`/teacher/adult-course-content-text-qa-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Multiple Choice")){
      history.push({
        pathname:`/teacher/adult-course-content-text-mcq-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Video")){
      history.push({
        pathname:`/teacher/adult-course-content-text-video-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Audio")){
      history.push({
        pathname:`/teacher/adult-course-content-text-audio-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Fill in the Blanks")){
      history.push({
        pathname:`/teacher/adult-course-content-text-blanks-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Handwriting")){
      history.push({
        pathname:`/teacher/adult-course-content-text-handwriting-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Digital Urdu")){
      history.push({
        pathname:`/teacher/adult-course-content-text-digitalUrdu-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "Text") && (data.answertype === "Drawing")){
      history.push({
        pathname:`/teacher/adult-course-content-text-drawing-edit-test/${id}`,
        state: data
      })
    }
    else if ((data.questiontype === "File Upload")){
      history.push({
        pathname:`/teacher/adult-course-content-file-upload-edit-test/${id}`,
        state: data
      })
    }
  }
